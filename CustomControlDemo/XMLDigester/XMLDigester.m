...
- (id) peekObjectAtIndex: (NSUInteger) aIndex
{
   id object = nil;
    NSUInteger index =  [stack_ count] - 1 - aIndex;
   if (index > 0 && index < [stack_ count]) { // TODO Do a better check
      object = [stack_ objectAtIndex:index];
   }
   return object;
}
